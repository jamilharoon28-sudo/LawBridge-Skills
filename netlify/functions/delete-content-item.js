...
function getSafeResourceAssetPath(value) {
  ...
  if (path.startsWith("public/uploads/resources/")) {
    return path;
  }
...
export async function handler(event, context) {
  ...
  if (collection === "resources") {
    const assetPath = getSafeResourceAssetPath(payload.assetPath);
    ...
    if (assetPath) {
      try {
        ...
        warnings.push(
          "The resource entry was deleted, but the uploaded file could not be deleted. It may already have been removed."
        );
      }
    }
  }
